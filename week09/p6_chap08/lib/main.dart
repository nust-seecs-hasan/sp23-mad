              ...
              builder: (BuildContext context,
                ...
                return ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int position) {
                      return ListTile(title: Text(heros.data![position]?.name));
                    });
              })),
    );
  }

  Future<List<hero.Hero>> readJsonFile() async {
    String myString = await DefaultAssetBundle.of(context)
        .loadString('assets/herosList.json');
    List myMap = jsonDecode(myString);

    List<hero.Hero> myHeros = [];
    myMap.forEach((dynamic hero) {
      hero.Hero myHero = hero.Hero.fromJsonMap(hero);
      myHeros.add(myHero);
    });
    return myHeros;
    // setState(() {
